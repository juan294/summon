...
describe("ensureGhostty", () => {
  it("exits when Ghostty.app is not found at any known path", () => {
    ...
    expect(() => ensureGhostty()).toThrow("process.exit");
...
describe("ensureAccessibility", () => {
  it("exits when accessibility check fails (osascript throws)", () => {
    ...
    expect(() => ensureAccessibility()).toThrow("process.exit");
    expect(mockExit).toHaveBeenCalledWith(1);

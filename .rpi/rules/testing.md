...
Run checks sequentially, never as parallel Bash calls (hook enforced). Summon's
canonical chain, matching `.husky/pre-commit` and `CLAUDE.md`, is:

```bash
pnpm typecheck && pnpm lint && pnpm build && pnpm test
```

The build is required because it catches build-only failures. A bare semicolon
chain hides earlier failures behind the last exit status.
